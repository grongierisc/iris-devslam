import redis, random, tqdm, colorama

def write(numKeys):
    redisConnection = redis.Redis()
    iter = tqdm.tqdm(range(numKeys), "Writing", bar_format='{l_bar}{bar}')
    for i in iter:
        redisConnection.set('key'+str(i), 'val'+str(random.randint(0,999999)))
...
def random_read(numKeys, numReads):
    ...
    for i in iter:
        redisConnection.get('key'+str(random.randint(0,numKeys)))
    return(iter.format_dict["elapsed"])
...
if __name__ == "__main__":
    numWrites = 50000
    numReads  = 75000
    print_results("Redis", "Writes", write(numWrites), numWrites)
    print_results("Redis", "Reads ", random_read(numWrites, numReads), numReads)
